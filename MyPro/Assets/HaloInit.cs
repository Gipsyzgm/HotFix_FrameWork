...
public class HaloInit : MonoBehaviour {
	...
	void Start () {
        ...
        theParticle.maxParticles = particleNum;
        theParticle.Emit(particleNum);
        theParticle.GetParticles(particles);
	...
	void Update () {
        ...
        theParticle.SetParticles(particles, particleNum);  
  
        ray = theCamera.ScreenPointToRay(Input.mousePosition);  
        if (Physics.Raycast(ray, out hit) && hit.collider.gameObject.name == "HaloCycle") ischange = true;  
        else ischange = false;  
	}
}
