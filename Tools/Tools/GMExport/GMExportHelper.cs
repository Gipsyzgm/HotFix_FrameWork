...
namespace Tools
{
    public class GMExportHelper
    {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (cellFirst.StartsWith("t_")) //一个新的表
                    {
                        ...
                        {
                            parseTabelInfo(tableInfo, fieldInfo, serverPath, Glob.codeOutSetting.GMServerDB.OutDBTableClassDir.ToReality());
                        ...
                        tableInfo[0] = cellFirst.Replace("t_", "");    //表名
            ...
            foreach (DataTable dt in ds.Tables)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (cellFirst.StartsWith("t_")) //一个新的表
                    {
                        if (tableInfo != null) //解析上一个表
                        {
                            parseTabelInfo(tableInfo, fieldInfo, serverPath, Glob.codeOutSetting.GMServerDB.OutDBGameTableClassDir.ToReality());
                            tabNames.Add(tableInfo);
                        }
                        bool isSId = cellFirst.EndsWith("$");
                        if (isSId)
        ...
        private static void parseTabelInfo(string[] tableInfo, List<string[]> fieldInfo, string serverPath,string filePath)
        {
            string saveFilePath = Path.Combine(filePath, tableInfo[0] + ".cs");
...
/// 工具生成，不要修改
/// </summary>
namespace GMServer.XGame.DBMgr
{{    
...
{fieldStrs.ToString()}       
        ...
        private static void createDBWriteTableFile(string serverPath, List<string[]> tabNames)
        {
            ...
            string saveFilePath = Glob.codeOutSetting.GMServerDB.OutDBWriteTableFile.ToReality();
...
{sbTable}        }}
        ...
        {{
         ...
         /// <summary>是否GM表</summary>
        public bool IsGMTable(string tableName)
        {{
            return tableNameDic.ContainsKey(tableName);
        }}
    }}
}}";
            Utils.SaveFile(saveFilePath, str, true);
        ...
        {
            string saveFilePath = Glob.codeOutSetting.GMServerDB.OutDBTableDBIndexFile.ToReality();
